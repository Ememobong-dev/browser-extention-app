      ...
      <div className="h-full w-full">
        <div className="flex flex-wrap gap-5 md:gap-0 justify-center md:justify-between  items-center h-full w-full">
          ...
          <div className="flex gap-3 items-center">
            <button onClick={() => handleAllButton()} className={` ${filtered==="all" ? "bg-red-500 text-neutral-900 font-bold" : `  ${themeDark ? "bg-neutral-700"  : "bg-neutral-100"} bg-neutral-700`}  py-2 px-5 cursor-pointer flex justify-center items-center rounded-full border border-neutral-500 focus:border-2 focus:border-red-500`}>
              All
            </button>
            <button onClick={() => handleActiveButton()} className={` ${filtered==="active" ? "bg-red-500 text-neutral-900 font-bold" : `  ${themeDark ? "bg-neutral-700"  : "bg-neutral-100"} bg-neutral-700`} py-2 px-5 cursor-pointer flex justify-center items-center rounded-full border border-neutral-500 focus:border-2 focus:border-red-500`}>
              Active
            </button>
            <button onClick={() => handleInactiveButton()} className={` ${filtered==="inactive" ? "bg-red-500 text-neutral-900 font-bold" : `  ${themeDark ? "bg-neutral-700"  : "bg-neutral-100"} bg-neutral-700`} py-2 px-5 cursor-pointer flex justify-center items-center rounded-full border border-neutral-500 focus:border-2 focus:border-red-500`}>
              Inactive
            </button>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-10">
          {dataExt.filter( (filteredItem) => filteredItem.isActive === isExtensionActive || isExtensionActive === null ).map((item) => (
            <div className="p-5 bg-neutral-800 border border-neutral-500 flex flex-col gap-5  rounded-3xl" key={item.name}>
              ...
              <div className="flex justify-between">
                <button className="py-1 px-2 cursor-pointer flex justify-center items-center rounded-full border border-neutral-500 focus:border-2 focus:border-red-500 hover:border-2 hover:font-semibold hover:border-red-500 hover:bg-red-500 hover:text-neutral-900">
                  Remove
                </button>
